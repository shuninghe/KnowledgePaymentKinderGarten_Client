...
const { setIgnoreType } = require('./src/gulpIgnorePage');
const srcPath = './src/**';
const distPath = './dist/';
...
const jsFiles = [`${srcPath}/*.js`,`${srcPath}/*.wxs`, `!${srcPath}/_template/*.js`, `!${srcPath}/env/*.js`];
const imgFiles = [
  `${srcPath}/images/*.{png,jpg,jpeg,gif,ico}`,
  `${srcPath}/images/**/*.{png,jpg,jpeg,gif,ico}`
];
/* 清除dist目录 */
gulp.task('clean', done => {
  del.sync(['dist/**/*','!dist/project.config.json']);
  done();
});
/* 编译wxml文件 */
const wxml = () => {
...
/* build */
gulp.task('build', gulp.series('clean', gulp.parallel( 'wxml', 'js', 'json', 'wxss', 'img', 'prodEnv'), 'watch'));
/* mock */
gulp.task('mock', gulp.series('clean', gulp.parallel( 'wxml', 'js', 'json', 'wxss', 'img', 'mockEnv'), 'watch'));
/* dev */
gulp.task('dev', gulp.series('clean', gulp.parallel( 'wxml', 'js', 'json', 'wxss', 'img', 'devEnv'), 'watch'));
/* test */
gulp.task('test', gulp.series('clean', gulp.parallel( 'wxml', 'js', 'json', 'wxss', 'img', 'testEnv'),'watch'));
/**
