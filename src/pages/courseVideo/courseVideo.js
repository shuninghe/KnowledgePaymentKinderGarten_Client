...
Page({
  data: {
    ...
    sourceList: [0, 0, 0],// 该课时是否含各资源类型[视频, 音频ppt, pdf, 图文链接]
    isComment: 0,         // 是否是点评类型
    curComment: {},       // 点评数据
  ...
  changeType (e) {
    ...
    if (typeof(e) !== 'object') {
      ...
      type = +e.currentTarget.dataset.type || +e.detail.type;
    }
    switch (type) {
    case 1:
  ...
  getCourseDetail (course_id) {
    return new Promise((resolve, reject) => {
      ...
      ajax(courseDetail, {data: params}).then(res => {
        if (res.code === 0) {
          let { product_id, pic, name, price, course_detail, course_resume, is_knowledge_map, isbuy, is_series, trees, validity } = res.data; 
          ...
          wx.setStorageSync('courseInfo', {
            product_id, course_id, name, pic, course_resume, price, course_detail, isbuy, is_knowledge_map
          });
  ...
  onShareAppMessage(e) {
    if (e.from === 'button') {
      ...
      return commonShare(
        trees[curPlayIndex].section_name,
        `pages/courseVideo/courseVideo?course_id=${courseInfo.course_id}&section_id=${section_id}&curPlayId=${curPlayId}&fromType=${sourceType}&fromShare=${true}`,
        trees[curPlayIndex].pic,
        true
      );
    } 
  },
