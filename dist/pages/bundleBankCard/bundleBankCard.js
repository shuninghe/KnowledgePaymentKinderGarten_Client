// 全局app实例
const { openBank, saveBankCard } = require('../../api.config');
...
Page({
  ...
  openBank () {
    ...
    ajax(openBank, {data:params, methods:'GET', falseToast: false}).then(res => {
      if (res.code === 0) {
        this.setData({
  ...
  saveBankCard () {
    ...
    ajax(saveBankCard, {data: params, method: 'POST'}).then(res => {
      if (res.code === 0) {
        wx.showToast({
          title: '绑定成功',
          icon: 'none',
