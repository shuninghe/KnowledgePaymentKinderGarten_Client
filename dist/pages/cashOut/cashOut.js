const { getCashInfo, applyCash } = require('../../api.config');
...
Page({
  ...
  // 获取提现信息
  getCashInfo() {
    ajax(getCashInfo, {failToast: false}).then(res=>{
      if (res.code === 0) {
        this.setData({
          cashInfo: res.data,
  ...
  succesCash() {
    ...
    ajax(applyCash, {data, method: 'post', failToast:false}).then(res => {
      if (res.code === 0) {
        wx.showToast({
          title: '提现成功',
          icon: 'none',
  ...
  selectBank() {
    wx.navigateTo({
      url: `/pages/choiceBankCard/choiceBankCard?cardId=${this.data.cashInfo.id}`,
    });
  },
  // 添加银行卡
  addBank() {
